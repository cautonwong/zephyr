...
1.  **Read-Before-Write**: NEVER use `cat >` or `write_file` to overwrite an existing source file unless the _entire_ content of the original file has been retrieved and verified in the current session.
...
- **Build for Hardware:**
  ...
  _Example:_ `./workflow.sh build vango_demo v32f20x_target`
...
_Note: This GEMINI.md takes precedence over general defaults for all development in /workspaces/applications._
